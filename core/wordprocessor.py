...
class MainWindow(QMainWindow):
    ...
    def __init__(self, *args, **kwargs):
     ...
     #   edit_menu.addAction(wrap_action)
       ...
       # format_menu = self.menuBar().addMenu("&Format")
        ...
        file_toolbar.addAction(QAction(QIcon('icons/lock'), 'Notebook', self, triggered=self.encrypt, checkable=True))
        file_toolbar.addAction(
            QAction(QIcon('icons/unlock'), 'Notebook', self, triggered=self.decrypt, checkable=True))
    ...
    def get_key(self):
        self.key, okPressed = QInputDialog.getText(self, " Key", "Key:", QLineEdit.Normal, "")
        if okPressed and self.key!= '':
            return self.key
    def file_name(self):
        self.f_name, okPressed = QInputDialog.getText(self, " Encrypted File ", "File Name:", QLineEdit.Normal, "")
        if okPressed and self.f_name !=  '':
            return self.f_name
    def get_dkey(self):
        self.dkey, okPressed = QInputDialog.getText(self, " Key", "Key:", QLineEdit.Normal, "")
        if okPressed and self.dkey !=  '':
            return self.dkey

    def file_dname(self):
        self.f_dname, okPressed = QInputDialog.getText(self, " Encrypted File ", "File Name:", QLineEdit.Normal, "")
        if okPressed and self.file_dname !=  '':
            return self.f_dname


    def encrypt(self,bufferSize):
        self.file_name()
        self.get_key()
        with open(self.f_name, 'w+') as fIn:
            fIn.write(self.editor.toPlainText())
            fIn.close()
        # encrypt
        with open(self.f_name, 'rb') as fIn:

            with open(self.f_name +'.aes', 'wb') as fOut:
                pyAesCrypt.encryptStream(fIn, fOut, self.key, self.bufferSize)
                remove(self.f_name)
        # get encrypted file size
        self.encFileSize = stat(self.f_name +'.aes').st_size



    def decrypt(self,bufferSize):
        self.file_dname()
        self.get_dkey()
        if self.f_dname == self.f_name and self.dkey == self.key:
            # decrypt
                with open(self.f_name + '.aes', "rb") as fIn:
                    try:
                        with open(self.f_name, "wb") as fOut:
                            # decrypt file stream
                            pyAesCrypt.decryptStream(fIn, fOut, self.key, self.bufferSize, self.encFileSize)
